...
const Payment = () => {
  ...
  const { appointmentId } = state || {}; // Access appointment ID passed from MyAppointments
  ...
  const handleCompletePayment = async () => {
    try {
      ...
      // Mark payment as completed in backend or local state
      const response = await axios.post('/api/user/complete-payment', { appointmentId });

      if (response.data.success) {
        setPaymentCompleted(true);
        // Redirect to MyAppointments page after successful payment
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="bg-white p-6 rounded-lg shadow-lg text-center max-w-md">
        ...
        <button
          onClick={handleCompletePayment} // Ensure the onClick function is properly attached
