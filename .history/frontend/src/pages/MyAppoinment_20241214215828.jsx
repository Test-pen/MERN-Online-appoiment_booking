...
const MyAppoinment = () => {
  ...
  const [appointments, setAppointments] = useState([]); // Default value is an empty array
        ...
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:gap-6 py-2 border-b'key={index}> 
            ...
            <div className='flex-1 text:sm text-zinc-600'>
              ...
              <p className='text-xs'>{item.docDataaddress.line1}</p>
              <p className='text-xs'>{item.docData.address.line2}</p> {/* Changed line1 to line2 here */}
              <p className='text-xs mt-1'><span className='text-sm text-neutral-700 font-medium'>Date & Time: </span> {item.slotDate }| 7:30 PM</p>
            </div>
