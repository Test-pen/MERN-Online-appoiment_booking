...
const DoctorDashboard = () => {
  ...
  return dashData && (
    <div className='m-5'>
      ...
      <div className='bg-white flex items-center gap-2.5 px-4 py-4 mt-10 rounded-t border'>
  ...
  {dashData.latestAppointments.map((item, index) => (
    <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={index}>
      ...
      {item.cancelled ? (
        ...
        <img onClick={() => cancelAppointment(item._id)} className='w-10 cursor-pointer' src='/assets/cancel-icon.png' alt='Cancel' />
      )}
    </div>
  ))}
</div>
